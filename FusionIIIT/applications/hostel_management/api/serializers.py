...
class HostelFineSerializer(serializers.ModelSerializer):
    ...
    hall_name = serializers.CharField(source='hall.hall_name', read_only=True)
    # Show the student_id that was entered, or fall back to student_roll_no
    student_id_display = serializers.SerializerMethodField(read_only=True)

    class Meta:
        model = HostelFine
        fields = ['fine_id', 'student', 'student_roll_no', 'student_id_entered', 'student_id_display', 'hall', 'hall_name', 'student_name', 'amount', 'status', 'reason']
        read_only_fields = ['fine_id']

    def get_student_id_display(self, obj):
        """Return the entered student_id if available, otherwise the actual student roll number"""
        return obj.student_id_entered if obj.student_id_entered else obj.student.id.id
