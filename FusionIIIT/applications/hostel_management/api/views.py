...
def create_notice(request):
    ...
    # Extract only the fields the service expects (exclude hall_id)
    notice_data = {k: v for k, v in serializer.validated_data.items() if k != 'hall_id'}
    notice = services.create_notice(hall=hall, posted_by=request.user.extrainfo, **notice_data)
...
def create_leave(request):
    ...
    # Auto-populate student info from authenticated user
    data = request.data.copy()
    data['student_name'] = request.user.first_name or request.user.username
    data['roll_num'] = request.user.username
...
def file_complaint(request):
    """File a complaint."""
    # Auto-populate student and hall info from authenticated user
    data = request.data.copy()
    data['student_name'] = request.user.first_name or request.user.username
    data['roll_number'] = request.user.username

    # Get hall_name from user's hostel role
    role_info = selectors.get_user_hostel_role(request.user)
    data['hall_name'] = role_info.get('hall_name', 'Unknown Hall') if role_info else 'Unknown Hall'
...
def impose_fine(request):
    ...
    # Auto-populate hall_id from user's hostel role or use first hall as default
    data = request.data.copy()
    role_info = selectors.get_user_hostel_role(request.user)
    if role_info and role_info.get('hall'):
        data['hall_id'] = role_info['hall']
    elif 'hall_id' not in data:
        # If no hall_id provided and user has no role, use first hall
        first_hall = selectors.get_all_halls().first()
        if first_hall:
            data['hall_id'] = first_hall.id

    serializer = ImposeFineSerializer(data=data)
    if not serializer.is_valid():
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
    try:
